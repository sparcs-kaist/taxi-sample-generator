const { generateUser, generateRoom, generateSampleLocations, generateChats } = require('./src/testData');
...
const main = async () => {
  ...
  for (const index of Array(numberOfRooms).keys()) {
    const roomOid = await generateRoom(sampleLocationOids, index + 1, 1, userOids[0]); //하드코딩: 일주일 뒤에 출발하는 방(들)을 만듭니다.
